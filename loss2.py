...
class SimilarityLoss(nn.Module):
    ...
    def forward(self, true, pred):  
        ...
        if self.similarity_type == "all":
            cosine_loss = torch.mean(1/ self.cosine_loss(true.to(torch.float32), pred.to(torch.float32))) 
            dist_loss = torch.mean(self.dist_loss(true.to(torch.float32), pred.to(torch.float32)))
            #print(cosine_loss, dist_loss)
            loss = cosine_loss + dist_loss
            
        elif self.similarity_type == "cosine":
            loss = torch.mean(1/ self.cosine_loss(true.to(torch.float32), pred.to(torch.float32)))
        else:
            loss = torch.mean(self.dist_loss(true.to(torch.float32), pred.to(torch.float32)))
    ...
    def forward(self,y1_true, y2_true, y1_pred, y2_pred, lstm_output=None, final_decoder_layer_output= None, eval=False): #with_logits=False,
        ...
        if lstm_output is not None and self.similarity is not None:
            ...
            if torch.all(y1_true.ne(0)):
                similarity_loss = self.similarity(lstm_output.squeeze(),final_decoder_layer_output.squeeze()) * 0
            else:
                mask = y1_true.eq(0)
